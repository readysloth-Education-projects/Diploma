...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[%
            ...
            \node [draw, rectangle, minimum height = 5cm, minimum width = 6cm] (qemu) {QEMU};
            \node [draw,
                   rectangle,
                   minimum height = 1.5cm,
                   minimum width = 6cm,
                   color = red] (qemu device) at (0,-1.75) {Виртуальное устройство};
            \node [draw,
                   rectangle,
                   minimum height = 1.5cm] (server) [right = 8cm of qemu device] {Узел обработки};

            \draw [->, color = blue] (qemu device.north east) -- (server.north west)
                                  node [midway, above, sloped] (info) {Данные, собранные устройством};
            \draw [->, color = blue] (server.south west) -- (qemu device.south east)
                        node [midway, above, sloped] (result) {Результат обработки};
